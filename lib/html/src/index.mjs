  ...
  const rawTemplate = strings
    .reduce((acc, curr, i) => {
      ...
      if (
        ...
        placeholders.set(id, Array.isArray(value) ? value : [value]);
        return acc.concat(curr, `<span id="${id}"></span>`);
      }

      if (Array.isArray(value) && value.every((item) => item instanceof Node)) {
      }
      return acc.concat(curr, sanitize(String(value)));
    }, "")
    .trim();
...
if (import.meta.vitest) {
  ...
  describe("html", () => {
    ...
    it("should apply array of outer passed elements", () => {
      const children = [
        html`<div id="child"></div>`,
        html`<div id="childTwo"></div>`,
      ];
      const result = html`<div class="element">${children}</div>`;
      expect(result.querySelector("#child")).toEqual(children[0]);
      expect(result.querySelector("#childTwo")).toEqual(children[1]);
    });

    it("should sanitize &", () => {
      const child = "x & y";
      const result = html`<div class="element">${child}</div>`;
      expect(result.innerHTML).toBe("x &amp; y");
    });
    ...
    it("should sanitize script include", () => {
      const child = `<script>alert('xss')</script>`;
      const result = html`<div class="element">${child}</div>`;
